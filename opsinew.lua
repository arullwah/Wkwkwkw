...
-- ========= IMPROVED RECORDING STUDIO GUI (180x110) =========
...
local ResumeBtn = CreateStudioBtn("▶️ RESUME", 3, 31, 164, 26, Color3.fromRGB(40, 150, 80))
...
-- ========= IMPROVED PLAYBACK CONTROL GUI (220x190) =========
local PlaybackControl = Instance.new("Frame")
PlaybackControl.Size = UDim2.fromOffset(220, 190)
PlaybackControl.Position = UDim2.new(0.5, -110, 0.3, 0)
...
PlaybackTitle.Text = "⚙️ PLAYBACK CONTROL"
...
ClosePlaybackBtn.Size = UDim2.fromOffset(22, 22)
ClosePlaybackBtn.Position = UDim2.new(1, -24, 0.5, -11)
...
local function CreateModernToggle(text, x, y, w, h, default)
    ...
    label.BackgroundTransparency = 1
    label.Text = (default and "✓ " or "✗ ") .. text
    ...
    local function UpdateToggle(isOn)
        ...
        label.Text = (isOn and "✓ " or "✗ ") .. text
    end
    
    return btn, UpdateToggle
end

local PlayBtnControl = CreatePlaybackBtn("▶️ PLAY", 2, 2, 200, 36, Color3.fromRGB(59, 15, 116))

local LoopBtnControl, AnimateLoopControl = CreateModernToggle("AutoLoop", 2, 44, 98, 26, false)
local ShiftLockBtnControl, AnimateShiftLockControl = CreateModernToggle("ShiftLock", 104, 44, 98, 26, false)

local RespawnBtnControl, AnimateRespawnControl = CreateModernToggle("AutoRespawn", 2, 74, 98, 26, false)
local ResetBtnControl, AnimateResetControl = CreateModernToggle("ResetChar", 104, 74, 98, 26, false)

local JumpBtnControl, AnimateJumpControl = CreateModernToggle("InfiniteJump", 2, 104, 200, 26, false)
...
local function CreateSpeedBtn(text, speed, x, y, w)
    local btn = Instance.new("TextButton")
    btn.Size = UDim2.fromOffset(w, 20)
    btn.Position = UDim2.fromOffset(x, y)
    ...
    btn.MouseButton1Click:Connect(function()
        CurrentSpeed = speed
        SpeedLabel.Text = "⚡ Speed: " .. string.format("%.2fx", speed)
...
CreateSpeedBtn("0.5x", 0.5, 2, 151, 38)
CreateSpeedBtn("0.75x", 0.75, 42, 151, 38)
CreateSpeedBtn("1x", 1.0, 82, 151, 38)
CreateSpeedBtn("2x", 2.0, 122, 151, 38)
CreateSpeedBtn("5x", 5.0, 162, 151, 38)
...
local PlaybackBtn = CreateButton("⚙️ PLAY", 158, 2, 76, 30, Color3.fromRGB(59, 15, 116))
...
local PathToggleBtn = CreateButton("🛣️ RUTE", 0, 96, 115, 30, Color3.fromRGB(59, 15, 116))
...
-- ========= NEW IMPROVED RECORD LIST LAYOUT =========
function UpdateRecordList()
    ...
    for index, name in ipairs(RecordingOrder) do
        ...
        -- Row 1: [Play] [Delete] ☑ Checkpoint1 [Up] [Down]
...
-- ========= COMPLETELY FIXED AUTO LOOP SYSTEM V3 =========
...
print("✅ ByaruL Recorder v2.3 - LOADED")
print("📝 Recording: 60 FPS with Time Bypass")
print("▶️ Playback: 60 FPS Smooth & Natural Speed")
print("🔄 Auto Loop: Fixed - No Stuck, Always Continue")
print("💀 Death: Auto-continues after respawn")
print("🎯 State: Optimized for natural animations")
print("🎨 UI: Modern layout with toggle buttons")
