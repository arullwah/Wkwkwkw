...
local function SaveHumanoidState()
    ...
    if humanoid then
        ...
        if prePauseHumanoidState == Enum.HumanoidStateType.Climbing then
            -- Keep the character in climbing state when paused
            humanoid.PlatformStand = false
...
-- ========= IMPROVED PLAYBACK SYSTEM WITH CLIMBING FIX =========
...
-- ========= FIXED AUTO LOOP SYSTEM WITH CLIMBING SUPPORT =========
...
function PausePlayback()
    if AutoLoop and IsAutoLoopPlaying then
        ...
        if IsPaused then
            ...
            print("Auto Loop paused - Proper state restored")
        else
    ...
    elseif IsPlaying then
        ...
        if IsPaused then
            ...
            PauseBtnBig.BackgroundColor3 = Color3.fromRGB(200, 50, 60)
            RestoreHumanoidState() -- FIX: Use proper state restoration for climbing
            print("Playback paused - Proper state restored")
        else
            PauseBtnBig.Text = "PAUSE"
            PauseBtnBig.BackgroundColor3 = Color3.fromRGB(180, 140, 40)
            SaveHumanoidState()
...
local function SaveToObfuscatedJSON()
    ...
    local success, err = pcall(function()
        ...
        saveData.Checkpoints = nil -- Hapus yang clear text
    ...
    local success, err = pcall(function()
        ...
        if saveData.Obfuscated and saveData.ObfuscatedFrames then
            ...
            for _, checkpointData in ipairs(saveData.Checkpoints or {}) do
                ...
                RecordedMovements[name] = deobfuscatedData[name]
                table.insert(RecordingOrder, name)
            ...
            for _, checkpointData in ipairs(saveData.Checkpoints or {}) do
                local name = checkpointData.Name
                RecordedMovements[name] = checkpointData.Frames
                table.insert(RecordingOrder, name)
                checkpointNames[name] = checkpointData.DisplayName or checkpointData.Name
            end
        end
