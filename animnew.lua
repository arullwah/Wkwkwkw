...
local function CreateMiniRecorderGUI()
    ...
    -- PERBAIKAN: Urutan button yang benar - <<⏪️ di kiri, ⏩️>> di kanan
    ...
    -- PERBAIKAN: Set emoji dengan urutan yang benar
    setIcon(btnPrev , "⏪️", "<<")
    setIcon(btnSave , "💾", "S")
    setIcon(btnRec  , "🎦", "R")
    setIcon(btnPause, "⏸", "||")
    ...
    return gui, {
        Save = btnSave,
        Record = btnRec,
        Previous = btnPrev,  -- PERBAIKAN: Nama yang sesuai
        Pause = btnPause,
        Next = btnNext       -- PERBAIKAN: Nama yang sesuai
    }
end
...
local function CreatePlaybackGUI()
    ...
    -- PERBAIKAN: Menggunakan emoji hijau untuk play, merah untuk stop
    local btnPlay  = makeButton("Play")
    local btnPause = makeButton("Pause")
    btnPlay.Parent  = row
    btnPause.Parent = row

    setIcon(btnPlay , "🟢", "PLAY")   -- PERBAIKAN: Hijau untuk play
    setIcon(btnPause, "🔴", "STOP")   -- PERBAIKAN: Merah untuk stop
...
local function ApplyFrameToCharacter(frame)
    ...
    task.spawn(function()
        ...
        if hum then
            -- PERBAIKAN: Tidak langsung set WalkSpeed ke 0 untuk menghindari interpolation walk
...
-- ========= IMPROVED PLAYBACK WITH SMOOTH TRANSITIONS =========
function PlayRecording(name)
    ...
    -- PERBAIKAN: Smooth transition dari posisi saat ini ke frame terdekat
    if distance <= RESUME_DISTANCE_THRESHOLD then
        ...
        -- Buat smooth transition dari posisi saat ini ke frame terdekat
        ...
        if (currentCFrame.Position - targetCFrame.Position).Magnitude > 1 then
            -- Interpolasi smooth ke posisi target
            local transitionTime = 0.3 -- 0.3 detik untuk transisi
            local startTime = tick()
    ...
    -- PERBAIKAN: Update floating GUI dengan emoji yang benar
    if PlaybackGUI and PlaybackGUI.Enabled then
        PlaybackGUI.Play.Text = "🔴"  -- Merah untuk stop saat sedang play
    end
    ...
    playbackConnection = RunService.Heartbeat:Connect(function(deltaTime)
        if not IsPlaying then
            ...
            -- PERBAIKAN: Kembalikan ke hijau saat stop
            if PlaybackGUI and PlaybackGUI.Enabled then
                PlaybackGUI.Play.Text = "🟢"  -- Hijau untuk play
            end
            return
        end
        ...
        while playbackAccumulator >= PLAYBACK_FIXED_TIMESTEP do
            ...
            task.spawn(function()
                -- PERBAIKAN: Smooth application of frame data
                local targetCFrame = GetFrameCFrame(frame)
                local currentCFrame = hrp.CFrame
                
                -- Smooth transition untuk rotasi, tapi instant untuk posisi (untuk akurasi)
                if (targetCFrame.Position - currentCFrame.Position).Magnitude < 10 then
                    -- Interpolasi rotasi saja untuk smoothness
                ...
                if hum then
                    -- PERBAIKAN: Tidak langsung set WalkSpeed ke 0, gunakan nilai dari frame
            ...
            while AutoLoop and IsAutoLoopPlaying and currentFrame <= #recording and deathRetryCount < maxDeathRetries do
                ...
                if IsPaused then
                    ...
                    while loopAccumulator >= PLAYBACK_FIXED_TIMESTEP do
                        ...
                        if frame then
                            task.spawn(function()
                                -- PERBAIKAN: Smooth application untuk auto loop juga
                                local targetCFrame = GetFrameCFrame(frame)
                                local currentCFrame = hrp.CFrame
            ...
            if distance <= RESUME_DISTANCE_THRESHOLD then
                ...
                playbackConnection = RunService.Heartbeat:Connect(function(deltaTime)
                    ...
                    while playbackAccumulator >= PLAYBACK_FIXED_TIMESTEP do
                        ...
                        task.spawn(function()
                            -- PERBAIKAN: Smooth application untuk resume juga
                            local targetCFrame = GetFrameCFrame(frame)
                            local currentCFrame = hrp.CFrame
...
FloatingBtn.MouseButton1Click:Connect(function()
    ...
    if not MiniRecorderGUI then
        ...
        -- PERBAIKAN: Connect floating recorder buttons dengan urutan yang benar
        ...
        -- PERBAIKAN: Previous button (⏪️) di kiri
        ...
        -- PERBAIKAN: Next button (⏩️) di kanan  
    ...
    if not PlaybackGUI then
        ...
        -- PERBAIKAN: Connect floating playback buttons dengan emoji yang benar
        PlaybackButtons.Play.MouseButton1Click:Connect(function()
            PlaySound("Click")
            if IsPlaying or IsAutoLoopPlaying then
                StopPlayback()
                PlaybackButtons.Play.Text = "🟢"  -- Kembali ke hijau saat stop
            else
                if AutoLoop then
                    StartAutoLoopAll()
                else
                ...
                PlaybackButtons.Play.Text = "🔴"  -- Merah saat sedang play
            end
        end)
